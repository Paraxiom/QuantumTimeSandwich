...
mod tests {
    ...
    fn test_state_initialization() {
        ...
        assert_eq!(*state_machine.current_state(), UnitCircleState::default(), "State machine should be initialized with the default state");
    }
    #[test]
    fn test_apply_gate() {
        let mut state_machine = UnitCircleStateMachine::new();
        // Set initial state to a GateOperation state
        let initial_state = UnitCircleState::gate_operation(GateType::PauliX, Complex::new(1.0, 0.0));
        ...
        assert_ne!(state_machine.current_state(), &initial_state, "State should change after applying the gate");
    }
    ...
    fn test_apply_protocol() {
        ...
        assert_ne!(state_machine.current_state(), &initial_state, "State should change after applying the protocol");
    }
    ...
    fn test_empty_protocol_application() {
        ...
        let protocol = Protocol::new(vec![]);
        state_machine.apply_protocol(&protocol);
        assert_eq!(state_machine.current_state(), &initial_state, "State should remain unchanged after applying an empty protocol");
    }
    ...
    fn test_protocol_with_multiple_gates() {
        ...
        state_machine.apply_protocol(&protocol);
        assert_ne!(state_machine.current_state(), &initial_state, "State should change after applying a protocol with multiple gates");
    }

    fn test_state_consistency() {
        let mut state_machine = UnitCircleStateMachine::new();
        println!("Initial state machine state: {:?}", state_machine.current_state()); // Debugging
        ...
        println!("State machine state after zero rotation: {:?}", state_machine.current_state()); // Debugging
    
        // Check if the state remains unchanged after applying a zero rotation gate.
        assert_eq!(state_machine.current_state(), &UnitCircleState::Initial, "State should remain unchanged after applying a zero rotation gate");
    }
