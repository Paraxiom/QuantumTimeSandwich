...
pub struct LocalBuilder<P: Precision> {
    ...
    measurements: usize,
    state_machine: UnitCircleStateMachine,
}

impl<P: Precision> LocalBuilder<P> {
    /// Creates a new `LocalBuilder`.
    ///
    /// # Returns
    ...
    pub fn new() -> Self {
        LocalBuilder {
            ...
            pipeline: Vec::new(),      // Assuming pipeline is a Vec
            n: 0,                      // Assuming n is an integer
            zeroed_qubits: Vec::new(), // Assuming zeroed_qubits is a Vec
            measurements: 0,           // Assuming measurements is an integer
            state_machine: UnitCircleStateMachine::new(), // Assuming this is the correct way to initialize
        }
        // Note: No semicolon here, to return the constructed LocalBuilder
    }
    // New method for updating the state machine based on gate type
    /// Updates the state machine based on the given gate type.
    ///
    /// This method applies the specified quantum gate operation to the state machine,
    /// which updates the internal state of the machine accordingly.
    ///
    /// # Arguments
    /// * `gate_type` - The type of quantum gate to apply.
    pub fn update_state_machine(&mut self, gate_type: GateType) {
        match gate_type {
            GateType::PauliX
            | GateType::PauliY
            | GateType::PauliZ
            | GateType::Hadamard
            | GateType::CNOT => {
                self.state_machine.apply_gate(gate_type);
            } // Add cases for other gate types as needed
        }
    }
    /// Simulates the application of a quantum gate using a state machine.
    ///
    /// # Arguments
    /// * `qubit` - Mutable reference to the qubit on which the gate will be applied.
...
impl Qudit {
    ...
    /// Updates the state of the qubit
    pub fn update_state(&mut self, new_state: UnitCircleState) {
...
impl<P: Precision> CircuitBuilder for LocalBuilder<P> {
    ...
    {
        let n = self.n();
        println!("Number of qubits (n): {}", n); // Added print statement

        let mut state = vec![Complex::zero(); 1 << n];
        let arena = state.clone();
        println!("Before calculating initial_index");
        ...
        it.into_iter()
            ...
            .for_each(|(index, bit)| {
                if index >= usize::BITS as usize {
                    println!("Skipped: index {} >= {}", index, usize::BITS); // Added print statement
                } else {
                    initial_index |= bit << index;
                }
            });

        println!("Initial index: {}", initial_index); // Added print statement
