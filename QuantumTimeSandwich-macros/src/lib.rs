    ...
    loop {
        ...
        for (ri, (rs, is)) in register_list.iter().zip(index_list.iter()).enumerate() {
            ...
            let full_string = Some("None.into_iter()".to_string()).into_iter().chain(rs.iter().zip(is).map(|(r, s)| {
                if let Some(s) = s {
                    format!("QuantumTimeSandwich::macros::program::QubitIndices::from({}).into_iter().map(|i| {}[i].take().unwrap())", s, r)
                } else {
        ...
        if control {
            ...
            if let Some(control_bits) = control_bits {
                ...
                output_stream.extend(TokenStream::from_str("let _program_register_0 = QuantumTimeSandwich::macros::program::negate_bitmask(_program_builder, _program_register_0, _control_bitmask);"));
